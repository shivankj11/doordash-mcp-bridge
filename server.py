...
TOOL_SPECS = [
    ...
    },
    {
        ...
        "description": (
            ...
            "DEFAULT MODIFIERS: set an item's `default_handling` to `exact` when its "
            "nested_options array is the complete desired selection, including [] when "
            "the requester wants no defaults. Omit it to keep defaults for modifier "
            "groups not included in nested_options.\n\n"
            "WEIGHT-PRICED RETAIL: use a decimal quantity for items sold by weight, "
            "such as 0.5 for half a pound. First read the item's measurement_unit and "
            "increment from build-grocery-list or item-details; do not assume pounds or "
            "a 0.5 increment. Confirm the returned cart line has purchase_type "
            "PURCHASE_TYPE_MEASUREMENT before describing it as a weight-priced item.\n\n"
